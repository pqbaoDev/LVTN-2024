...
const Stockindetail = ({ stockIn, open, handleClose, stopPropagation }) => {
        ...
        <div>
            {open ? (
                ...
                >
                    <div className="border-b border-gray-800  flex py-2 pl-3 items-center   gap-2 ">
                        <div className="w-4 h-4 p-0">
                            <img src={detailing} alt="" />
                                    ...
                                    <h2 className="text-[16px] font-normal">Chi tiết phiếu nhập</h2>
                    </div>
                                ...
                                <div key={index} className="grid grid-cols-2 gap-3">
                                    ...
                                    <div className="w-[435px]">
                                        <div className="grid grid-cols-5">
                                            ...
                                            <p className="col-span-3">{stockIn?.employee?.name}</p>
                                        </div>
                                        <div className="grid grid-cols-5">
                                            <p className="font-semibold text-[14px] col-span-2">Số điện thoại:</p>
                                            <p className="col-span-3">{stockIn?.employee?.phone}</p>
                                        </div>
                                                                       
                                                                        <div className="grid grid-cols-5">
                                        <p className="font-semibold text-[14px] col-span-2">Ngày nhập:</p>
                                        <p>{FormatDay(stockIn.date)}</p>
                                                                        </div>
                                                                        <div className="grid grid-cols-5">
                                            <p className="font-semibold text-[14px] col-span-2">Trạng thái:</p>
                                            <p className="col-span-3">{stockIn.status }</p>
                                        </div>
                                                                        <div className="flex items-center gap-10">
                                        <div className="grid grid-cols-5">
                                            <p className="font-semibold text-[14px] col-span-2">Khu:</p>
                                            <p className="text-[16x] font-bold col-span-3">{stockIn.location.zone.symbol}</p>
                                        </div>
                                        
                                        <div className="grid grid-cols-5">
                                            <p className="font-semibold text-[14px] col-span-2">Kệ/Pallet:</p>
                                            <p className="text-[16x] font-bold col-span-3">{stockIn.location.type === 'rack' ? stockIn.location.rack : stockIn.location.pallet }</p>
                                        </div>
                                        {
                                            stockIn.location.type === 'rack' && (
                                                <div className="flex items-center gap-2">
                                            <p className="font-semibold text-[14px] col-span-2">Tầng:</p>
                                            <p className="text-[16x] font-bold col-span-3">{stockIn.location.level }</p>
                                        </div>
                                            )
                                        }
                                                                        </div>
                                        ...
                                        {stockIn.products.map((item, index) => {
                                            ...
                                            return (
                                                <tr key={index}>
                                                    ...
                                                    <td className="bg-white  border-2 border-solid border-indigo-500 uppercase p-1">{stockIn._id}</td>
                    ...
                    {print && <PrintStockIn stockIn={stockIn} />}
